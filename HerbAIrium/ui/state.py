...
def initialize_session_state():
    ...
    if 'llm_base_url' not in st.session_state:
        st.session_state.llm_base_url = "https://api.deepinfra.com/v1/openai/chat/completions"
    if 'deepinfra_api_key' not in st.session_state:
        st.session_state.deepinfra_api_key = ""
    ...
    if 'llm_parse_model' not in st.session_state:
        st.session_state.llm_parse_model = "openai/gpt-oss-120b"
    if 'llm_parse_temperature' not in st.session_state:
        st.session_state.llm_parse_temperature = 0.7
    if 'llm_parse_max_tokens' not in st.session_state:
        st.session_state.llm_parse_max_tokens = 4096
    if 'llm_parse_prompt' not in st.session_state:
        st.session_state.llm_parse_prompt = "The following is a transcription of a herbarium specimen image. Please parse the transcription and extract the following information: collector name, location, family, and collection date. Return the information in a JSON format."
    ...
    # LLM parse results storage (indexed by image path)
...
def save_configuration():
    ...
    config_data = {
        ...
        "deepinfra_api_key": st.session_state.deepinfra_api_key,
        ...
        "llm_parse_model": st.session_state.llm_parse_model,
        "llm_parse_temperature": st.session_state.llm_parse_temperature,
        "llm_parse_max_tokens": st.session_state.llm_parse_max_tokens,
        "llm_parse_prompt": st.session_state.llm_parse_prompt,
    }
...
def load_configuration():
    ...
    try:
        ...
        if "llm_base_url" in config_data:
            st.session_state.llm_base_url = config_data["llm_base_url"]
        if "deepinfra_api_key" in config_data:
            st.session_state.deepinfra_api_key = config_data["deepinfra_api_key"]
        ...
        if "llm_parse_model" in config_data:
            st.session_state.llm_parse_model = config_data["llm_parse_model"]
        if "llm_parse_temperature" in config_data:
            st.session_state.llm_parse_temperature = config_data["llm_parse_temperature"]
        if "llm_parse_max_tokens" in config_data:
            st.session_state.llm_parse_max_tokens = config_data["llm_parse_max_tokens"]
        if "llm_parse_prompt" in config_data:
            st.session_state.llm_parse_prompt = config_data["llm_parse_prompt"]
